    ...
    $project: {
        ...
        total_money: {
            $function: {
                ...
                body: 'function (a) {      \n          let string_to_parse = a;\n          currency = string_to_parse.match("^([^0-9]+)")[0];\n          currencies = {\n            "$": "USD",\n            "€": "EUR",\n            "£": "GBP",\n            "C$": "CAD"\n          }\n          raw_amount = string_to_parse.replace(currency, "");\n          let last_char = raw_amount[raw_amount.length - 1];\n          if (isNaN(last_char)) {\n            raw_amount = raw_amount.replace(\'k\', \'E3\').replace(\'M\', \'E6\').replace(\'B\', \'E9\');\n            raw_amount = parseFloat(raw_amount);\n          } else {\n            raw_amount = raw_amount.replace(\'.\', "").replace(\',\', \'\');\n          }\n          currency = currencies[currency]\n\n              return {"amount": raw_amount, "currency": currency};\n      }'
            }
        }
    }
}]